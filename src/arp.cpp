...
Arp::Arp(const std::vector<uint8_t> &src)
    : hw_type(src[0] << 8 | src[1]), proto_type(src[2] << 8 | src[3]),
      hlen(src[4]), plen(src[5]), op(src[6] << 8 | src[7]),
...
const std::array<uint8_t, 4> &Arp::get_dest_ip() const
{
    return dest_ip;
}
const std::array<uint8_t, 6> &Arp::get_src_mac() const
{
    return src_mac;
}
